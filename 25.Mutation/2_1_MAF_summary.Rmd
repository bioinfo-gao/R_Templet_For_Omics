...
conda install conda-forge/label/cf202003::r-devtools
...
conda install bioconda::bioconductor-maftool -y
```{r eval=FALSE, include=FALSE}
# install.packages("magick")
#install.packages("plot3D")
# install.packages("tidyplots") # install.packages("Matrix")   conda install r-matrix -y 
# install.packages("wordcloud") # conda install -c conda-forge r-wordcloud -y
install.packages('R.utils')
# install.packages("remotes")
# library(remotes)
# install_version("barplot3d", version = "1.0", repos = "https://cran.r-project.org/src/contrib/Archive/barplot3d/")
# 
# install.packages("plot3D") # shift Alt Down 
...
Each of these functions and a short description is summarized as shown below. Usage is simple, just read your MAF file with `read.maf` (along with copy-number data if available) and pass the resulting MAF object to the desired function for plotting or analysis.
...
Instead of an `MAF` a custom data can also be used for plotting.  
Input should be a two column data frame with pos and counts.
...
注： 高度相关的突变簇，各突变之间距离很短，不在簇上的随机突变总体上距离很长
...
`tcgaCompare` uses mutation load from TCGA 
...
GISTIC（Genomic Identification of Significant Targets in Cancer）分析的结果，展示整个基因组中复发性体细胞拷贝数变异 (SCNA) 的显著性。
...
FAB 分类 (French-American-British Classification)：
含义：FAB 分类是一种传统的、基于形态学（细胞外观）和细胞化学染色结果的方案，主要用于对急性白血病进行分类，特别是急性髓系白血病 (AML) 和急性淋巴细胞白血病 (ALL)。
常见亚型（以 AML 为例）：M0（未分化型）、M1（髓母细胞型未成熟）、M2（髓母细胞型成熟）、M3（早幼粒细胞型）、M4（粒单核细胞型）、M5（单核细胞型）、M6（红白血病）、M7（巨核细胞型）。
...
可视化单个样本的拷贝数变异 (Copy Number Variation, CNV) 区域，这些区域是通过 CBS (Circular Binary Segmentation) 算法从原始的 DNA 测序数据（如 log2比例值）中分割出来的。

Y 轴表示测序深度在肿瘤样本与对照样本（或预设的正常拷贝数）之间的 
log2 / log2 比值。

(肿瘤样本拷贝数 / 正常样本拷贝数)

Y = 0：中性拷贝数。表示该区域的拷贝数是正常的（通常假设为 2 拷贝）。
Y > 0：拷贝数增加（扩增）。例如，如果 Y≈0.58，则 Ratio ≈ 1.5
Y < 0：拷贝数减少（缺失）。例如，如果 Y≈−1.0，则 Ratio ≈ 0.5，表示 1 拷贝（单拷贝丢失）。如果Y  值更低，可能表示纯合缺失
...
### Lollipop plot-2  >> 棒棒糖图 (Lollipop Plot)
...
含义：图上的**垂直线（棒）和顶部的圆圈（糖）**代表一个体细胞突变事件。
棒的高度：棒棒糖的高度通常表示该位点发生的突变数量（即在该氨基酸位点上发生突变的不同样本数）。棒越高，该位点越“热点”（Hotspot）。
圆圈颜色：圆圈的颜色是最重要的信息之一，它根据突变的类型进行编码。
...
lollipopPlot2(m1 = primary.apl, m2 = relapse.apl, 
              gene = "PML", 
              AACol1 = "amino_acid_change", AACol2 = "amino_acid_change", 
...
```{r gistic-plot-rotated, echo=FALSE, fig.cap="旋转90度的gisticOncoPlot", fig.show='hide'}
# 1. 正常生成图片 (knitr 会将图保存到默认路径)

setwd("~/Code/R_code/R_Templet_For_Omics/25.Mutation")

png("./figure/gistic-plot-rotated-1.png", width = 800, height = 1200, res = 300)
...
```{r rotate-and-insert, echo=FALSE, message=FALSE, warning=FALSE}
# library(magick)
# 
# # 2. 找到 knitr 刚刚生成的图片文件路径
# # knitr::fig_chunk() 获取当前 Rmd 块生成的图片路径 (假设只有一张图)
# fig_path <- knitr::fig_chunk("gistic-plot-rotated", "png") 
# 
# # 3. 读取、旋转并覆盖保存
# img <- image_read(fig_path)
# rotated_img <- image_rotate(img, degrees = 90)
# image_write(rotated_img, path = fig_path, format = "png") 
# 
# # 4. 插入旋转后的图片
...
左侧 0  到 1：代表非目标组（即 Rest 组）中的突变频率。
右侧 0  到 1：代表目标组（即某个特定的 FAB 亚型）中的突变频率。
注意：X轴是对称的，中心点是0
...
#plot3D::plot3D(contextdata = sig1, labels = FALSE, scalexy = 0.01, sixcolors = "sanger", alpha = 0.5)
```
...
-   `somaticInteractions` plot is inspired from the article [Combining gene mutation with gene expression data improves outcome prediction in myelodysplastic syndromes](https://www.nature.com/articles/ncomms6901). Thanks to authors for making source code 
